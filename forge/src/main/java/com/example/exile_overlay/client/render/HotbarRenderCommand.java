...
/**
 * ホットバー描画コマンド。
 * レイヤー構造: Background → Fill(オーブ液面) → Frame → Overlay(反射・テキスト・スロット)。
...
public class HotbarRenderCommand implements IRenderCommand {
    ...
    private void renderLevelDisplay(GuiGraphics graphics, Minecraft mc) {
        EquipmentDisplayConfig.LevelDisplayMode mode = equipConfig.getLevelDisplayMode();
    ...
    private void renderPotionSlot(GuiGraphics graphics, Minecraft mc) {
        ...
        ItemStack hpPotion = MethodHandlesUtil.findBestPotion(mc.player, true);
        ItemStack manaPotion = MethodHandlesUtil.findBestPotion(mc.player, false);
        boolean hpCooldown = MethodHandlesUtil.isPotionOnCooldown(mc.player, hpPotion);
