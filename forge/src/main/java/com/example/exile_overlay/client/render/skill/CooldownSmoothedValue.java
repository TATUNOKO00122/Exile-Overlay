...
public final class CooldownSmoothedValue {
    ...
    /**
     * チャージスキルのリジェネ割合を滑らかに補間して取得
     */
    public static float getSmoothedChargeRegen(int slot, float targetPercent) {
        if (slot < 0 || slot >= SLOT_COUNT) {
            return targetPercent;
        }
        return computeSmoothed(REGEN_STATES[slot], targetPercent, 0);
    }
