...
import com.example.exile_overlay.compat.GatewaysCompat;
import com.example.exile_overlay.compat.MineAndSlashTrackerClientCompat;
...
/**
 * HUDレンダリングマネージャー。
 * RenderPipeline管理・フレームカウンター更新・全HUD要素の統合管理点。
 */
public class HudRenderManager {
    ...
    private final Map<String, IRenderCommand> rendererIndex = new ConcurrentHashMap<>();
    ...
    private void registerDefaultCommands() {
        ...
        // ミニオンオーバーレイ
        this.registerCommand(new MinionOverlayRenderer(), 50);
        ...
        // Boss HPバー（画面上部固定）
        this.registerCommand(new BossHpBarRenderer(), 90);

        // Gateway Boss Bar（GatewaysToEternity連携）— Gateways導入時のみ登録
        if (ModList.get().isLoaded("gateways")) {
            GatewaysCompat.register(this);
        }
        ...
        if (ModList.get().isLoaded("mmorpg")) {
            MineAndSlashTrackerClientCompat.registerRenderer(this);
        }
    ...
    public void registerCommand(IRenderCommand command, int priority) {
        ...
        rendererIndex.put(command.getConfigKey(), command);
    ...
    public boolean unregisterCommand(String commandId) {
        ...
        if (command != null) {
            rendererIndex.remove(command.getConfigKey());
        }
     ...
     * @return IRenderCommand、見つからない場合はnull
     */
    public IRenderCommand getHudRenderer(String configKey) {
        if (configKey == null) {
            return null;
        }
