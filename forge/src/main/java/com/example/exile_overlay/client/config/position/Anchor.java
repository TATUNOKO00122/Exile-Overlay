...
public enum Anchor {
    ...
    public static Anchor estimateFromPosition(int x, int y, int screenWidth, int screenHeight) {
        ...
        int quarterW = screenWidth / 4;
        int quarterH = screenHeight / 4;

        boolean left = x <= centerX - quarterW;
        boolean right = x >= centerX + quarterW;
        boolean top = y <= centerY - quarterH;
        boolean bottom = y >= centerY + quarterH;
