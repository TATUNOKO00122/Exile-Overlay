...
public class ArmorDurabilityRenderer implements IRenderCommand {
    ...
    private static final int ESTIMATED_TEXT_WIDTH = 48;
    private static final int ESTIMATED_WIDTH = PADDING_X + ITEM_SIZE + GAP + ESTIMATED_TEXT_WIDTH + PADDING_X;
    ...
    public void render(GuiGraphics graphics, RenderContext ctx) {
        ...
        // 表示するアイテムを収集
        List<ItemStack> itemsToRender = new ArrayList<>(MAX_ITEMS);
        ...
        // ドラッグ設定画面中で装備が全くない場合はダミーアイテムを表示
        if (itemsToRender.isEmpty() && mc.screen instanceof DraggableHudConfigScreen) {
            itemsToRender.add(new ItemStack(Items.DIAMOND_HELMET));
            itemsToRender.add(new ItemStack(Items.DIAMOND_CHESTPLATE));
        }
    ...
    private void renderItem(GuiGraphics graphics, Minecraft mc, ItemStack stack, int y, int centerX) {
        // パディング(PADDING_X)を考慮してアイテムアイコンを描画
        int itemLeftX = centerX - ESTIMATED_WIDTH / 2 + PADDING_X;
        graphics.renderItem(stack, itemLeftX, y);
        ...
        if (stack.isDamageableItem()) {
            ...
            String text;
            if (equipConfig.isUsePercentage()) {
                text = String.format("%.0f%%", durabilityPercent * 100);
            } else {
                text = String.format("%d/%d", remaining, maxDamage);
            }

            int textX = itemLeftX + ITEM_SIZE + GAP;
            boolean shadow = equipConfig.isEnableShadow();
            if (shadow) {
                RenderSystem.disableBlend();
                HudFontHelper.drawString(graphics, mc.font, text, textX, y + 4, color, true);
                RenderSystem.enableBlend();
            } else {
                HudFontHelper.drawString(graphics, mc.font, text, textX, y + 4, color, false);
            }
        }
    }
