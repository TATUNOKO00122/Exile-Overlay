...
/**
 * 全HUDデータのキャッシュを一元管理するシングルトン。
 * フレームカウンターベースのTTLを持ち、UUIDを直接キーにすることでアロケーションを回避している。
 * データ構造: Player UUID → DataType → CachedValue
 */
public class UnifiedCache {
    ...
    // UUIDを直接キーとしてString変換を省略（ゼロアロケーション）
    private final Map<UUID, Map<DataType, CachedValue>> playerCaches = new ConcurrentHashMap<>();
    
    private final AtomicLong globalFrameCounter = new AtomicLong(0);
    ...
    public void incrementFrame() {
        globalFrameCounter.incrementAndGet();
    }
    ...
    public float get(Player player, DataType type, Supplier<Float> fetcher) {
        ...
        return cv.getOrCompute(fetcher, globalFrameCounter.get());
    }
    ...
    public CacheStats getStats() {
        ...
        int totalEntries = playerCaches.values().stream()
            .mapToInt(Map::size)
            .sum();
        return new CacheStats(playerCount, totalEntries, globalFrameCounter.get());
    }
