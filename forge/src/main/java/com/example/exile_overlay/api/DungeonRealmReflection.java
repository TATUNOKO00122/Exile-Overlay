...
public final class DungeonRealmReflection {
    ...
    /**
     * プレイヤーのScoreboardのダンジョン情報表示を非表示にする
     * dungeon_realm MODが作成した「completion_percent」Objectiveの
     * サイドバー表示のみをキャンセルし、Objective自体は削除しない
     */
    public static void clearScoreboard(Player player) {
        ...
        try {
            ...
            if (objective != null) {
                ...
                LOGGER.debug("Hidden dungeon_realm scoreboard sidebar for player: {}", player.getName().getString());
            }
        } catch (Throwable t) {
            LOGGER.debug("Error hiding scoreboard: {}", t.getMessage());
        }
    }
}