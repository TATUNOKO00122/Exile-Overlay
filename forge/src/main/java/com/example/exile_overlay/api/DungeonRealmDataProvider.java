 ...
 * キル達成率・ルート達成率・レアリティティア・各種キル数などダンジョン関連データを提供する。
 * DungeonMapCapability(ワールド保存データ) および DungeonMapData(現在のダンジョン状態)から取得する。
 */
public class DungeonRealmDataProvider extends AbstractModDataProvider {
     ...
     * dungeon_realm MODがScoreboardに表示する「Map Stats」のサイドバー表示をキャンセルする
     * Objective自体は削除せず、表示のみを非表示にする
     */
